...
public class UserRegisterServlet extends GenericServlet {
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		String wallet = req.getParameter(IUserContants.COLUMN_WALLET);
		try {
			Connection con = DBConnection.getCon();
			PreparedStatement ps = con
					.prepareStatement("insert into " + IUserContants.TABLE_USERS + " (username,password, firstname,lastname,address,phone,mailid,usertype,wallet) values(?,?,?,?,?,?,?,?,?)");
			...
			rd.include(req, res);
			pw.println("<div class=\"tab\">Incorrect Details Entered, Please Register Again</div>");
			e.printStackTrace();
		}
	}
}
